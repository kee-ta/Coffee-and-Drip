...
public class Brewer : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D other)
    {
        ...
        if (other.gameObject.GetComponent<GroundedCoffeeBeans>())
        {
            ...
            if (brewStack.Count < maxBrewSlots)
            {
                ...
                slotFilled?.Invoke();
                Debug.Log(brewStack.Peek().GetComponent<GroundedCoffeeBeans>().GetName());
            }
        }
    }
    ...
    private void OnMouseDown()
    {
        if (brewStack.Count > 0)
        {
            ...
            brewStack.Pop().GetComponent<Rigidbody2D>().AddForce(new Vector2(0.2f, 6f), ForceMode2D.Impulse);;
        }
    }
